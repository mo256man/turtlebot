...
def getKey(joykey, key_timeout):
    tty.setraw(sys.stdin.fileno())
    rlist, _, _ = select.select([sys.stdin], [], [], key_timeout)
    if rlist:
        key = sys.stdin.read(1)
    else:   # キー入力なかったら、空文字ではなくジョイスティックのキー値とする
        # key = ""
        key = joykey
    ...
    try:
        ...
        while True:
            ...
            if key in moveBindings.keys():
                ...
                # コマンドがなかったら停止させる　ただしすでに止まっている場合は何もしない
                if x == y == z == th == 0:
                    continue                # ループ処理をスキップする　つまりコマンドを配信しない
                x, y, z, th = 0, 0, 0, 0
